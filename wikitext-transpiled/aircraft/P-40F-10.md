...
The '''{{Specs|name}}''' is a rank {{Specs|rank}} American fighter {{Battle-rating}}. It was introduced in [[Update 1.79 "Project X"]].

The {{Specs|name}} is a pursuit plane: its role is to intercept enemy fighters and bombers. The F-10 differs noticeably from the previous P-40 variants on 3 points:

* a new engine, the Packard-built Merlin engine that performs better at high altitudes,
* a longer tail to compensate the extra torque (the plane is 26 inches / 66 cm longer),
* the upper nose air intake is removed (while the lower nose air intake gets bigger).
